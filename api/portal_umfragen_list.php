...
try {
    ...
    // Alle aktiven + geschlossene (mit eigenen Antworten) Umfragen
    $role = $_SESSION['user_role'] ?? 'mitglied';
    $stmt = $db->prepare("
        SELECT u.id, u.titel, u.beschreibung, u.gueltig_bis, u.status,
        ...
        ORDER BY
            ...
            u.erstellt_am DESC
    ");
    $stmt->execute([$mitglied_id, $mitglied_id, $role, $mitglied_id]);
