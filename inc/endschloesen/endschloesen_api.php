...
try {
    ...
    switch($action) {
        ...
        case 'get_selection':
            ...
            jsonResponse(true, $selected, '', ['zahlungsmethode' => $zahlungsmethode, 'zabig_partner' => $zabig_partner]);
            break;
        ...
        case 'save_selection':
            ...
            if ($gast_name && !$mitglied_id) {
                ...
                $stmt = $conn->prepare("SELECT id, geburtsdatum FROM endstich_gaeste WHERE name = ? AND jahr = ?");
                $stmt->bind_param("si", $gast_name, $jahr);
                $stmt->execute();
                $result = $stmt->get_result();
                ...
                if ($existing_gast) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO endstich_gaeste (name, geburtsdatum, jahr, created_by) VALUES (?, ?, ?, ?)");
                    $stmt->bind_param("ssis", $gast_name_clean, $geburtsdatum, $jahr, $created_by);
                    $stmt->execute();
            ...
            foreach ($details as &$entity) {
                if ($entity['typ'] === 'mitglied') {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        // Speichere sie_und_er Status für Zabig
                        if ($row['code'] === 'ZABIG' && $row['sie_und_er'] == 1) {
                            if (!isset($entity['partner_stiche'])) {
                                $entity['partner_stiche'] = [];
                            }
