...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($passe as $mitgliedID => $passen) {
        $resultateSql = "SELECT * FROM heimresultate WHERE MitgliedID = $mitgliedID AND Jahr = $jahr";
        ...
        if ($resultateResult->num_rows > 0) {
            ...
            if($hasAnyValue){
                $insertSql = "INSERT INTO heimresultate (MitgliedID, Jahr, Passe1, Passe2, Passe3, Passe4, Passe5, Passe6, Passe7, Passe8) VALUES ($mitgliedID, $jahr, ";
                for ($i = 1; $i <= 8; $i++) {
