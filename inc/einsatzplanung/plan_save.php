    ...
    if ($action === 'create' || $action === 'copy') {
        ...
        if ($action === 'copy') {
            ...
            $db->prepare("INSERT INTO einsatz_plaene (jahr, typ, titel, layout, status, fusstext, erstellt_von) VALUES (?, ?, ?, ?, 'entwurf', ?, ?)")
               ->execute([$jahr, $typ, mb_substr($titel, 0, 100), $layout, ep_fusstext_default($typ), $userId]);
            $planId = (int)$db->lastInsertId();

            // Termine aus JM-Definition (Obli/Feld)
            if ($ausJm) {
            ...
            // Startfunktionen für Obli/Feld (wie die bisherigen Dokumente); Chilbi: Einsatztypen
            $start = [];
            if ($typ === 'obligatorisch') {
