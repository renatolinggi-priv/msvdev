...
if (!defined('DB_HOST')) {
    // KORRIGIERT: config.php liegt in /inc/
    include_once dirname(dirname(__FILE__)) . '/config.php';
}

// function_date.php liegt in /inc/
if (file_exists(dirname(dirname(__FILE__)) . '/endsch_targetprint/function_date.php')) {
    include_once dirname(dirname(__FILE__)) . '/endsch_targetprint/function_date.php';
}
...
class PDFGenerator {
    ...
    public function __construct($conn, $year = null) {
        ...
        // Prüfen ob config_pdf.php existiert (in /inc/)
        $configPdfPath = dirname(dirname(__FILE__)) . '/config_pdf.php';
        if (file_exists($configPdfPath)) {
            include_once $configPdfPath;
            $this->useConfigPdf = true;
        }
        
        // Logo konvertieren - KORRIGIERT: Logo liegt in /inc/dat/
        $logoPath = dirname(dirname(__FILE__)) . '/endsch_targetprint/dat/MSVWilen_Logo.jpg';
     ...
     * KORRIGIERT: Speichert in /inc/dat/ statt lokalem dat/
     */
    protected function generatePDF($html, $filename, $orientation = 'portrait') {
        ...
        // KORRIGIERT: Speichere in /inc/dat/
        $pdfDir = dirname(dirname(__FILE__)) . '/dat';
        if (!file_exists($pdfDir)) {
            mkdir($pdfDir, 0755, true);
        }
        
        $pdfFilename = $filename . '_' . $date->format('Y-m-d_H-i-s') . '.pdf';
        ...
        // Relativer Pfad für Browser
        return 'inc/dat/' . $pdfFilename;
    }
