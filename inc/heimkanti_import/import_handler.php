...
try {
    if ($action === 'get_all_stich_definitions') {
        $db = db_mysqli();
        // Nur die 5 gewünschten Stiche
        $stmt = $db->prepare("
            SELECT stich, restable, nummer1, nummer2, nummer3
            FROM interne_stichdefinition
            WHERE stich IN ('Endstich','Kunst','Glück','Zabig','Schwini')
        ");
        $stmt->execute();
        $res = $stmt->get_result();
    ...
    if ($action === 'find_member_by_license') {
        // FIX: Unterstütze sowohl GET als auch POST
        $license = trim($_GET['license'] ?? $_POST['license'] ?? '');
        
        if (!$license) {
            json_err('license fehlt');
        }
        
        // Verhindere HTTP2 Probleme durch kleinere Response
        ...
        if (!$stmt) {
            error_log("[ENDSCH-DEBUG] Statement preparation failed: " . $db->error);
        ...
        if (!$result) {
            error_log("[ENDSCH-DEBUG] Query execution failed: " . $stmt->error);
            $stmt->close();
            json_err('Query error');
        }
        ...
        // Sende kompakte JSON Response
        if ($row) {
            error_log("[ENDSCH-DEBUG] Member found: ID=" . $row['ID'] . ", Name=" . $row['Vollname']);
            
            // Kompakte Response ohne zusätzliche Daten
            echo json_encode([
                'success' => true,
                'member_id' => (int)$row['ID'], 
                'member_name' => $row['Vollname']
        ...
        // Kompakte Response für "nicht gefunden"
        echo json_encode([
            'success' => true,
            'member_id' => null
        ]);
    ...
    if ($action === 'check_existing') {
        // Für heimkanti_import - prüft heimresultate und kantiresultate
        $mitglied_id = (int)($_GET['mitglied_id'] ?? 0);
        $jahr = (int)($_GET['jahr'] ?? date('Y'));
        $stich_type = trim($_GET['stich_type'] ?? 'Heimmeisterschaft');
        
        if (!$mitglied_id || !$jahr) json_err('Parameter fehlen');
        
        $db = db_mysqli();
        
        // Lade die Definition für den gewünschten Stich
        require_once 'import_handler_kanti.php';
        
        try {
            $result = checkExistingData($db, $mitglied_id, $jahr, $stich_type);
            json_ok($result);
        } catch (Exception $e) {
    ...
    if ($action === 'import_results') {
        // Für heimkanti_import - importiert in heimresultate oder kantiresultate
        if (!hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'] ?? '')) json_err('CSRF ungültig');

        $mitglied_id = (int)($_POST['mitglied_id'] ?? 0);
        $jahr = (int)($_POST['jahr'] ?? date('Y'));
        $stich_type = trim($_POST['stich_type'] ?? 'Heimmeisterschaft');
        $selected_programs_json = $_POST['selected_programs'] ?? '[]';
        
        if (!$mitglied_id || !$jahr || !$stich_type) json_err('Parameter unvollständig');
        
        $selected_programs = json_decode($selected_programs_json, true);
        if (!is_array($selected_programs)) json_err('Ungültige Programme');
        
        $db = db_mysqli();
        
        // Lade die Import-Funktion
        require_once 'import_handler_kanti.php';
        
        try {
            $result = handleImportResults($db, $mitglied_id, $jahr, $selected_programs, $stich_type);
            json_ok($result);
        } catch (Exception $e) {
            json_err($e->getMessage());
        }
