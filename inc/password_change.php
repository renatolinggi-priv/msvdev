    ...
    if (!password_verify($old_password, $current_password_hash)) {
        ...
        if (empty($password_errors)) {
            ...
            if ($update_stmt->execute()) {
                ...
                // Flag für erfolgreiche Änderung setzen
        ...
        <div class="card-body">
            ...
            <?php if (isset($success)): ?>
                ...
                <div class="text-center">
                    ...
                    <p class="text-muted">Sie werden in <span id="countdown">3</span> Sekunden weitergeleitet...</p>
                </div>
                <script>
                    // Success Animation und Weiterleitung
                    $(document).ready(function() {
                        $('.success-animation i').hide().fadeIn(500).addClass('pulse-animation');
                        ...
                        const countdownInterval = setInterval(function() {
                            ...
                            if (seconds <= 0) {
                                ...
                                // Zur vorherigen Seite oder Home
                                const referrer = document.referrer;
                                if (referrer && !referrer.includes('change_password') && !referrer.includes('password_change')) {
...
$(document).ready(function() {
    ...
    function updatePasswordStrength(password) {
        ...
        const username = '<?php echo $username; ?>'.toLowerCase();
        ...
        const notContainsUsername = !password.toLowerCase().includes(username);
        
        // Icons aktualisieren
        updateRequirementIcon('length', hasLength);
        updateRequirementIcon('uppercase', hasUppercase);
        updateRequirementIcon('lowercase', hasLowercase);
        updateRequirementIcon('numbers', hasNumbers);
        updateRequirementIcon('special', hasSpecial);
        updateRequirementIcon('username', notContainsUsername);
        
        // Kategorien zählen
        let categoriesCount = 0;
        if (hasUppercase) categoriesCount++;
        if (hasLowercase) categoriesCount++;
        if (hasNumbers) categoriesCount++;
        if (hasSpecial) categoriesCount++;
        
        updateRequirementIcon('categories', categoriesCount >= 3);
        
        // Stärke berechnen
        if (password.length === 0) {
            strengthMessage = 'Geben Sie ein Passwort ein...';
            strengthClass = '';
        ...
        if (categoriesCount >= 2) strength++;
        if (categoriesCount >= 3) strength++;
        if (categoriesCount === 4) strength++;
        if (notContainsUsername) strength++;
        
        const percentage = (strength / 5) * 100;
        
        if (!hasLength) {
            strengthMessage = 'Zu kurz - noch ' + (10 - password.length) + ' Zeichen';
            strengthClass = 'strength-weak';
            strengthText.removeClass('text-warning text-success text-muted').addClass('text-danger');
        } else if (!notContainsUsername) {
            strengthMessage = 'Passwort enthält Benutzernamen!';
            strengthClass = 'strength-weak';
            strengthText.removeClass('text-warning text-success text-muted').addClass('text-danger');
        } else if (categoriesCount < 3) {
            strengthMessage = 'Schwach - verwenden Sie mehr Zeichentypen';
            strengthClass = 'strength-weak';
            strengthText.removeClass('text-warning text-success text-muted').addClass('text-danger');
        } else if (percentage < 80) {
            strengthMessage = 'Mittel - schon gut, kann noch besser werden';
            strengthClass = 'strength-medium';
            strengthText.removeClass('text-danger text-success text-muted').addClass('text-warning');
        } else {
            strengthMessage = 'Stark - ausgezeichnetes Passwort!';
            strengthClass = 'strength-strong';
            strengthText.removeClass('text-danger text-warning text-muted').addClass('text-success');
        }
    ...
    // Anforderungs-Icon aktualisieren
    function updateRequirementIcon(requirement, fulfilled) {
        const element = $(`[data-requirement="${requirement}"]`);
        const icon = element.find('i').first();
        
        if (fulfilled) {
            element.addClass('fulfilled');
            icon.removeClass('bi-circle bi-x-circle')
                .addClass('bi-check-circle-fill');
        } else if ($('#new_password').val().length > 0) {
            element.removeClass('fulfilled');
            icon.removeClass('bi-circle bi-check-circle-fill')
                .addClass('bi-x-circle');
        } else {
            element.removeClass('fulfilled');
            icon.removeClass('bi-x-circle bi-check-circle-fill')
                .addClass('bi-circle');
        }
    }
    ...
    $('#new_password').on('input', function() {
        ...
        // Live-Validierung für Passwort-Bestätigung
    ...
    // Beim Fokus auf das Passwort-Feld
