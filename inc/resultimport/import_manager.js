...
const ImportManager = {
    ...
    displayProgramGroup(programs, programNumber, containerId, passeMapping) {
        ...
        programs.forEach((prog, idx) => {
            // Alle Programme sind auswählbar, aber nur die ersten 4 sind initial ausgewählt
            const selected = idx < 4 ? 'selected' : '';
            
            html += `
                <div class="program-selection-card ${selected}" 
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="text-end">
                            ...
                            <span class="passe-assignment" data-index="${idx}">-</span>
                        </div>
                    </div>
                </div>
            `;
        ...
        // Update Passe-Zuordnungen initial
        this.updatePasseAssignments(programNumber, passeMapping);
    },
    ...
    toggleProgramSelection(element) {
        ...
        // Update Passe-Zuordnungen basierend auf Auswahl
        const passeMapping = programNumber === '133' ? 
            ['Passe 1', 'Passe 3', 'Passe 5', 'Passe 7'] : 
            ['Passe 2', 'Passe 4', 'Passe 6', 'Passe 8'];
        this.updatePasseAssignments(programNumber, passeMapping);
        
        this.updateImportPreview();
    },
    
    updatePasseAssignments(programNumber, passeMapping) {
        // Hole alle ausgewählten Programme dieses Typs
        const selectedCards = $(`.program-selection-card[data-program="${programNumber}"].selected`);
        
        // Sortiere sie nach ihrem ursprünglichen Index
        const sortedCards = selectedCards.toArray().sort((a, b) => {
            return $(a).data('index') - $(b).data('index');
        });
        
        // Update alle Passe-Zuordnungen für dieses Programm
        $(`.program-selection-card[data-program="${programNumber}"]`).each(function() {
            const card = $(this);
            const passeSpan = card.find('.passe-assignment');
            
            if (card.hasClass('selected')) {
                // Finde die Position in den ausgewählten Karten
                const position = sortedCards.indexOf(this);
                if (position >= 0 && position < 4) {
                    passeSpan.text(passeMapping[position]);
                } else {
                    passeSpan.text('-');
                }
            } else {
    ...
    updateImportPreview() {
        ...
        // Programme 133 sammeln - sortiert nach ursprünglichem Index
        const selected133 = $('.program-selection-card[data-program="133"].selected').toArray()
            .sort((a, b) => $(a).data('index') - $(b).data('index'));
        
        selected133.forEach((elem, selIndex) => {
            const idx = $(elem).data('index');
            if (selIndex < 4) { // Nur die ersten 4 nach Sortierung
        ...
        // Programme 134 sammeln - sortiert nach ursprünglichem Index
        const selected134 = $('.program-selection-card[data-program="134"].selected').toArray()
            .sort((a, b) => $(a).data('index') - $(b).data('index'));
        
        selected134.forEach((elem, selIndex) => {
            const idx = $(elem).data('index');
            if (selIndex < 4) { // Nur die ersten 4 nach Sortierung
                this.selectedPrograms.push({
                    number: '134',
    ...
    executeImport() {
        const mitgliedId = $('#mitgliedSelect').val();
        const jahr = $('#jahrSelect').val();
        const mitgliedName = $('#mitgliedSelect option:selected').text();
        ...
        // Modal mit Details füllen
        let detailsHtml = `
            <div class="alert alert-info mb-3">
                <strong>Mitglied:</strong> ${mitgliedName}<br>
                <strong>Jahr:</strong> ${jahr}<br>
                <strong>Anzahl Programme:</strong> ${this.selectedPrograms.length}
            </div>
            <strong>Folgende Passen werden importiert:</strong>
            <ul class="mt-2">
        `;
        
        // Sortiere nach Passe für die Anzeige
        const passes = [];
        this.selectedPrograms.forEach(prog => {
            let passeNr;
            if (prog.number === '133') {
        ...
        passes.forEach(pass => {
            detailsHtml += `<li>Passe ${pass.passeNr}: Programm ${pass.program} (Total: ${pass.total})</li>`;
        });
        
        detailsHtml += '</ul>';
        
        $('#confirmImportDetails').html(detailsHtml);
        
        // Modal zeigen
        const modal = new bootstrap.Modal(document.getElementById('confirmImportModal'));
        modal.show();
        
        // Event Handler für Bestätigung (einmalig entfernen und neu setzen)
