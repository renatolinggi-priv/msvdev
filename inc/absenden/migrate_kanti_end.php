...
foreach ($jahre as $jahr) {
    ...
    if ($defResult->num_rows == 0) {
        echo "  âš ï¸  Keine Endstich-Definition für Jahr $jahr gefunden\n";
        $defStmt->close();
        continue;
    }
    $defRow = $defResult->fetch_assoc();
    $endstichDefID = $defRow['ID'];
    echo "  âœ“ Endstich Definition ID: $endstichDefID\n";
    $defStmt->close();
    ...
    while ($row = $endstichResult->fetch_assoc()) {
        ...
        if ($checkResult->num_rows > 0) {
            ...
            $updateStmt->close();
            if ($oldPunkte != $summe) {
                echo "  â†» Mitglied $mitgliedID: $oldPunkte â†’ $summe Punkte\n";
                $updated++;
            }
        } else {
    ...
    $endstichStmt->close();
    echo "  âœ“ $count Einträge verarbeitet ($inserted neu, $updated aktualisiert)\n";
}
...
foreach ($jahre as $jahr) {
    ...
    $defResult = $defStmt->get_result();
    if ($defResult->num_rows == 0) {
        echo "  âš ï¸  Keine Kantonalstich-Definition für Jahr $jahr gefunden\n";
        $defStmt->close();
        continue;
    }
    $defRow = $defResult->fetch_assoc();
    $kantiDefID = $defRow['ID'];
    echo "  âœ“ Kantonalstich Definition ID: $kantiDefID\n";
    $defStmt->close();
    ...
    while ($row = $kantiResult->fetch_assoc()) {
        ...
        if ($checkResult->num_rows > 0) {
            ...
            if ($oldPunkte != $bestePasse) {
                echo "  â†» Mitglied $mitgliedID: $oldPunkte â†’ $bestePasse Punkte\n";
                $updated++;
            }
        } else {
